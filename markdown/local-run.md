...
Running some notebooks in your local machine will require additional settings that should be run before launching the Jupyter Notebook.
...
- The function checks if the **environment variable** (e.g., `STPSF_PATH`) is set **and points to an existing directory**.
...
If you already have the data locally and don't want to re-download it, ensure you have set up the environment variables before starting the notebook.

For example, for `STPSF` you need to define `STPSF_PATH`.

In the command line or permanently in your shell profile (e.g., ~/.bashrc, ~/.zshrc):

```
export STPSF_PATH="/your/preferred/path/to/stpsf_data"
```

You can also add a cell in your notebook with the following code:

```
os.environ['STPSF_PATH'] = "/your/preferred/path/to/stpsf_data"
```

This cell should be run before the first code cell in the notebook. This is in particular useful if you re-run your notebook and you did not set this environment variable in your shell profile.
...
for the `STPSF_PATH` environment variable. Or in the notebook:

```
os.environ.pop('STPSF_PATH', None)
```
