...
def plot_rbt(doc):
    ...
    color_gradient = ["navy", "blue", "dodgerblue", "green", "orange", "red", "firebrick", "darkred"]
    default_filter = "F184"
    ...
    PANEL_W = 933       # width for controls & all three plots (change this to grow/shrink everything)
    PANEL_H = 520
    ...
    # ---------- VIOLIN PLOT ----------
    plot_violin = figure(
        title="Background Distribution by Filter",
    ...
    show_all_filters  = CheckboxGroup(labels=["Show all filters"], active=[])
    single_filter     = Select(title="Single filter", value=default_filter, options=list(overlay_filters.keys()))
    ...
    def toggle_visibility(attr, old, new):
        ...
        selected_filter_label.text = f"Filter: {selected}"
